...
import SidebarAtencion from './SidebarAtencion.vue';
...
function labelBoton(r) {
  if (r.estado === 'en_espera') return 'Atender';
  if (r.estado === 'atendida') return 'Ver';
  return null;
}
...
<template>
  <div class="flex items-stretch h-full">
    <div class="flex-1 min-w-0 overflow-auto px-10 py-8">
      <!-- Header -->
        ...
        <table class="w-full">
          ...
          <tbody>
            <tr
              ...
              :class="reservaActiva?.id === r.id && showAtencion ? 'bg-blue-50/70' : 'hover:bg-slate-50/70'"
            >
              <td class="px-6 py-4">
    ...
    <SidebarAtencion v-if="showAtencion" :reserva="reservaActiva" @close="showAtencion = false" />
  </div>
</template>
